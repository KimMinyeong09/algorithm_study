...
def sap(g, aList, bList):
    aVisited = [False for _ in range(g.V)]
    bVisited = [False for _ in range(g.V)]
    aDistance = [None for _ in range(g.V)]
    bDistance = [None for _ in range(g.V)]

    # Queue 클래스 객체 Q 만들어 aList, bList 모든 정점 추가
    # 추가는 3-tuple 형식: (정점번호, a or b, 거쳐온 거리 = 0)
    Q = Queue()
    for a in aList:
        Q.put((a, 'a', 0))
        aVisited[a] = True
        aDistance[a] = 0
        if a in bList: # 만약 aList와 bList 모두에 속하는 정점 v가 있다면 (v, 0)을 반환하고 종료
            return a, 0
    for b in bList:
        Q.put((b, 'b', 0))
        bVisited[b] = True
    ...
    while Q.qsize() > 0:
        v = Q.get()
        if v[2] >= sapLength: break
        for w in g.adj[v[0]]:
            if v[1] == 'a': # v가 aList로부터 왔다면
                if not aVisited[w]:
                    Q.put((w, 'a', v[2] + 1))
                    aVisited[w] = True
                    aDistance[w] = v[2] + 1
                if bVisited[w]:
                    if sapLength > aDistance[w] + bDistance[w]:
                        sapLength = aDistance[w] + bDistance[w]
                        sca = w
